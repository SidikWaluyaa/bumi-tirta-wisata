            ...
            <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="rating" value="{{ $service->rating ?? 5.0 }}" step="0.1" min="0" max="5"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#004AAD] focus:border-transparent transition-all">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="category" required 
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#004AAD] focus:border-transparent transition-all">
                        <option value="paket" {{ $service->category === 'paket' ? 'selected' : '' }}>Package</option>
                        <option value="addon" {{ $service->category === 'addon' ? 'selected' : '' }}>Add-on</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="price_unit" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#004AAD] focus:border-transparent transition-all">
                            <option value="/ Orang" {{ ($service->price_unit ?? '/ Orang') === '/ Orang' ? 'selected' : '' }}>/ Orang</option>
                            <option value="/ Unit" {{ $service->price_unit === '/ Unit' ? 'selected' : '' }}>/ Unit</option>
                            <option value="/ Paket" {{ $service->price_unit === '/ Paket' ? 'selected' : '' }}>/ Paket</option>
                            <option value="/ Set" {{ $service->price_unit === '/ Set' ? 'selected' : '' }}>/ Set</option>
                            <option value="/ Hari" {{ $service->price_unit === '/ Hari' ? 'selected' : '' }}>/ Hari</option>
                            <option value="" {{ empty($service->price_unit) ? 'selected' : '' }}>Tanpa Unit (Flat Price)</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" 
                        ...
                        placeholder="Brief description about this service...">{{ $service->description }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if($service->image)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $service->image) }}" class="w-full max-w-md h-64 object-cover rounded-xl border-2 border-gray-200">
                        </div>
                    @endif
