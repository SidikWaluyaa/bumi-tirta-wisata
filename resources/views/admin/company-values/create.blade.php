...
@section('content')
    <x-slot name="header">Add New Company Value</x-slot>

    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl shadow-lg p-6 lg:p-8">
            <form action="{{ route('admin.company-values.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">Use emoji or Heroicons name</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required 
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#004AAD] focus:border-transparent transition-all"
                        placeholder="e.g., Keamanan & Asuransi">
                    @error('title')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" required 
                        ...
                        placeholder="e.g., Semua peserta diasuransikan dengan safety equipment standar internasional">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
